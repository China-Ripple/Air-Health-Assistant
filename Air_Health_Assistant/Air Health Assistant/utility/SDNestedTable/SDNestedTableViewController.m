...
{
    NSLog(@"expandingItem:(SDGroupCell");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //NSMutableDictionary subItemsAmt,每个path中都有对应的子行数，存到dict中
    [subItemsAmt setObject:amt forKey:indexPath];
    
    NSLog(@"numberOfSubItemsforItem:cell %@,%@",amt,indexPath);
...
- (CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"heightForRowAtIndexPath %d,%d,%d,%@",[SDGroupCell getHeight],[SDGroupCell getsubCellHeight],amt,indexPath);
...
- (void) collapsableButtonTapped: (UIControl *) button withEvent: (UIEvent *) event
{
    ...
    NSLog(@"collapsableButtonTapped:%@",expandedIndexes);
}
